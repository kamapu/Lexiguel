...
lim_bars <- function(x, y, breaks = 10, pos = NULL, horiz = FALSE,
	...
	if(horiz) {
		...
		for(i in 1:(length(breaks) - 1)) {
			do.call(polygon, c(list(y = c(rep(breaks[i], 2),
											rep(breaks[i + 1], 2)),
									x = c(min(y[x_class == i]),
		...
		if(!is.null(pos))
			do.call(segments, c(list(y0 = breaks[-length(breaks)],
									x0 = trend, y1 = breaks[-1]), pos_args))
	} else {
		plot(x = x, y = y, type = "n", ...)
		for(i in 1:(length(breaks) - 1)) {
			do.call(polygon, c(list(x = c(rep(breaks[i], 2),
											rep(breaks[i + 1], 2)),
									y = c(min(y[x_class == i]),
											rep(max(y[x_class == i]), 2),
											min(y[x_class == i]))),
		...
		if(!is.null(pos))
			do.call(segments, c(list(x0 = breaks[-length(breaks)],
									y0 = trend, x1 = breaks[-1]), pos_args))
	}
}
